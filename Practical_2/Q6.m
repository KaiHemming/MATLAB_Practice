...
close all;
detectShapes(original, LB, numFills, COLOUR_DATA, 50, SHAPE_DATA, 1);
close all;

[original, im, LB, numFills] = countCloseBlocks(2, 4600, 3, 88, 1);
close all;
detectShapes(original, LB, numFills, COLOUR_DATA, 60, SHAPE_DATA, 2);
close all;

[original, im, LB, numFills] = countCloseBlocks(3, 4600, 3, 80, 4);
close all;
detectShapes(original, LB, numFills, COLOUR_DATA, 60, SHAPE_DATA, 3);
close all;

[original, im, LB, numFills] = countCloseBlocks(4, 4600, 3, 80, 4);
close all;
detectShapes(original, LB, numFills, COLOUR_DATA, 60, SHAPE_DATA, 4);
close all;

[original, im, LB, numFills] = countCloseBlocks(5, 4600, 3, 40, 1);
close all;
detectShapes(original, LB, numFills, COLOUR_DATA, 60, SHAPE_DATA, 5); 
close all;
...
function detectShapes(original, LB, numFills, COLOUR_DATA, COLOUR_VARIANCE_VALUE, SHAPE_DATA, IM_NUM)
    % Taken values from original image instead of colour quantised image
    % Improved accuracy.
    red = original(:,:,1);
    green = original(:,:,2);
    blue = original(:,:,3);
    itemNum = 0; % For saving images

    outputString = "Image " + IM_NUM + ", Shapes:";

    for i = 2:numFills % Skip the first one (the carpet, close to 0,0,0)
        im = LB == i;
        meanRed = mean(red(im));
        meanGreen = mean(green(im));
        meanBlue = mean(blue(im));
        ...
        foundShape = colourName + " " + SHAPE_DATA(shapeIndex,1);
        outputString = outputString + " " + foundShape + ",";

        figure
        imshow(original(row1:row2, col1:col2, :));
        title(foundShape);
        f = gcf;
        exportgraphics(f, "Q6/" + IM_NUM + "_" + itemNum + ".png");
        itemNum = itemNum + 1;

    end
    fileID = fopen("Q6/" + IM_NUM + ".txt", "w");
    fprintf(fileID,'%s.\n', outputString);
